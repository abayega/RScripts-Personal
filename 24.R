...
raw_count <- read.table("combined",header=T,row.names=1)

gene_abs_exp <-raw_count[93:nrow(raw_count),]
gene_abs_exp <- cbind(gene_abs_exp$TPE_1H,gene_abs_exp$TPE_2H,gene_abs_exp$TPE_3H,gene_abs_exp$TPE_4H,gene_abs_exp$TPE_5H,gene_abs_exp$TPE_6H)

gene_abs_exp <- data.matrix(gene_abs_exp)
colnames(gene_abs_exp) <- c('TPE_1H','TPE_2H','TPE_3H','TPE_4H','TPE_5H','TPE_6H')
rownames(gene_abs_exp2) <- rownames(raw_count)[93:nrow(raw_count)]
boxplot.matrix(gene_abs_exp, use.cols = T,ylim = c(1200,250000),ylab="Transcripts per embryo",col = c('red', 'green', 'blue', 'gray','yellow','purple'),
               main = 'Absolute gene expression accross timepoints')

##Find which gene is the most highly expressed
gene_abs_exp2 <- data.frame(gene_abs_exp)
rownames(gene_abs_exp2)[match(max(gene_abs_exp2$TPE_1H),gene_abs_exp2$TPE_1H)]

k <- apply(gene_abs_exp,2,out <- function(x) {
       max(x)
   })

#k
#TPE_1H   TPE_2H   TPE_3H   TPE_4H   TPE_5H   TPE_6H 
#38815196 40856809 71119208 52227579 72263114 58862873 
rownames(gene_abs_exp2)[gene_abs_exp2$TPE_1H == 38815196] #Not sure why this does not work



###010918
##Let us try to reproduce the results we did in test9
rm(list = ls())
setwd("/home/abayega/R/tests/test24")
ont_exprn <-read.table('combined_ONT_gene_expression', header = T, row.names = 1, sep = '\t')
ont_exprn_erccs <- read.table('ERCC_ont_expression', header = T, row.names = 1, sep = '\t')
ill_exprn <-read.table('rsem_gene_counts.txt', header = T, row.names = 1, sep = '\t')
...
lm_eqn = function(df1){
  df1 = subset(df1, rownames(df1)!='ERCC-00116',select=c(fem, mal_TPM))
  print(length(rownames(df1)))
  m = lm(log2(mal_TPM) ~ log2(fem), df1);
...
#Ploting correlation of ONTs 1H
ont_exprn_erccs_1H = subset(ont_exprn_erccs, X1H_TPM >= 0.01,select=c(X1H_exp, X1H_TPM))
ggplot(data = ont_exprn_erccs_1H, aes(x = log2(X1H_exp), y = log2(X1H_TPM))) +
  geom_point() +
  geom_smooth(method = "lm") +
  ggtitle("correlation expected vs RPG10K Bo_E_1H ONT") +
  ...
  my.theme
#Ploting correlation of ONTs 2H
ont_exprn_erccs_1H = subset(ont_exprn_erccs, X2H_TPM >= 0.01,select=c(X2H_exp, X2H_TPM))
ggplot(data = ont_exprn_erccs_1H, aes(x = log2(X2H_exp), y = log2(X2H_TPM))) +
  geom_point() +
  geom_smooth(method = "lm") +
  ggtitle("correlation expected vs RPG10K Bo_E_2H ONT") +
  scale_x_continuous(expression(log[2]*" Transcript molecules")) +
  scale_y_continuous(expression(log[2]*" RPG10K")) +
  annotate(geom = "text", x = 16, y = 6, label = lm_eqn(ont_exprn_erccs_1H), parse = T, color = "red",size=6) +
  my.theme
#Ploting correlation of ONTs 3H
ont_exprn_erccs_1H = subset(ont_exprn_erccs, X3H_TPM >= 0.01,select=c(X3H_exp, X3H_TPM))
ggplot(data = ont_exprn_erccs_1H, aes(x = log2(X3H_exp), y = log2(X3H_TPM))) +
  geom_point() +
  geom_smooth(method = "lm") +
  ggtitle("correlation expected vs RPG10K Bo_E_3H ONT") +
  scale_x_continuous(expression(log[2]*" Transcript molecules")) +
  scale_y_continuous(expression(log[2]*" RPG10K")) +
  annotate(geom = "text", x = 16, y = 6, label = lm_eqn(ont_exprn_erccs_1H), parse = T, color = "red",size=6) +
  my.theme
#Ploting correlation of ONTs 4H
ont_exprn_erccs_1H = subset(ont_exprn_erccs, X4H_TPM >= 0.01,select=c(X4H_exp, X4H_TPM))
ggplot(data = ont_exprn_erccs_1H, aes(x = log2(X4H_exp), y = log2(X4H_TPM))) +
  geom_point() +
  geom_smooth(method = "lm") +
  ggtitle("correlation expected vs RPG10K Bo_E_4H ONT") +
  scale_x_continuous(expression(log[2]*" Transcript molecules")) +
  scale_y_continuous(expression(log[2]*" RPG10K")) +
  annotate(geom = "text", x = 16, y = 6, label = lm_eqn(ont_exprn_erccs_1H), parse = T, color = "red",size=6) +
  my.theme
